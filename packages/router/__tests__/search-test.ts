...
import type { SearchHistory } from "@remix-run/router";
import { createSearchHistory } from "@remix-run/router";
...
describe("a search history", () => {
  ...
  it("knows how to create hrefs from location objects", () => {
    ...
    expect(href).toEqual("?the=query&page=%2Fthe%2Fpath#the-hash");
  });

  it("knows to merge the current search string into the generated href when usings objects", () => {
    dom.window.history.replaceState(null, "", "?a-param=the-value");
    history = createSearchHistory({
      window: dom.window as unknown as Window,
      param: "page",
      merge: true,
    ...
    expect(href).toEqual(
      "?the=query&page=%2Fthe%2Fpath&a-param=the-value#the-hash"
    );
  });
  ...
  it("knows to merge the current search string into the generated href when using strings", () => {
    dom.window.history.replaceState(null, "", "?a-param=the-value");
    history = createSearchHistory({
      window: dom.window as unknown as Window,
      param: "page",
  ...
  it("supports a custom page param", () => {
    history = createSearchHistory({
      window: dom.window as unknown as Window,
      param: "my-page-param",
    });
    const href = history.createHref("/the/path?the=query#the-hash");
    expect(href).toEqual("?the=query&my-page-param=%2Fthe%2Fpath#the-hash");
  });
